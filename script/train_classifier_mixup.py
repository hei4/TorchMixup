...
from util import MixupDataset


def onehot_encode(label, n_class=10):
    diag = torch.eye(n_class)
    oh_vector = diag[label].view(-1, n_class)
    return oh_vector
...
def main():
    ...
    train_cifar = torchvision.datasets.CIFAR10(root='../cifar10_root', train=True, download=True, transform=train_trans)
    train_set = MixupDataset(train_cifar)
    ...
    for epoch in range(epoch_size):  # loop over the dataset multiple times
        ...
        for itr, data in enumerate(train_loader):
            # get the inputs
            image, max_label, oh_label = data

            train_true.extend(max_label.tolist())

            image, oh_label = image.to(device), oh_label.to(device)   # for GPU
            ...
            logit = net(image)
            loss = -torch.mean(log_softmax(logit) * oh_label)
    ...
    df.to_csv('train_wmix.log', index=False)
